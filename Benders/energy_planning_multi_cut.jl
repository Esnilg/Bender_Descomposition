...
@variable(master,θ[1:NS] >= -1e10/NS);
...
@objective(master, Min, sum((tau[i] * c_inv[i] + c_maint[i]) * F_Mult[i] for i=TECHNOLOGIES)  + sum(θ[s] for s=1:NS))
...
while(true)
	...
	LB = obj + sum(variable_value[θ[s]] for s=1:NS)
	...
	if abs(UB-LB) < 0.01
		...
		for s =1:NS
			@constraint(master, θ[s] >= probability[s]*objective_y[s] + sum(probability[s]*dual_F[s][i]*(F_Mult[i] - variable_value[F_Mult[i]]) for i=TECHNOLOGIES) + probability[s]*dual_SH_DHN[s]*(Share_Heat_Dhn - variable_value[Share_Heat_Dhn]))
		end
	end
	...
	#CSV.write("df_Bender_multi_cut_$(nScenarios).csv",df_simulations,delim=';', decimal='.');
